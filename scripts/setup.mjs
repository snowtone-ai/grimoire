...
if (detectUiSurface()) {
  ...
  // No chrome-devtools MCP here: Playwright MCP is already registered
  // globally (user scope) and in active use for browser verification on
  // this operator's projects. impeccable itself prefers the harness's
  // existing browser tool over a dedicated MCP (see its
  // reference/critique.md), and its own URL-scan engine falls back to a
  // bundled Puppeteer, not chrome-devtools. Adding a second overlapping
  // browser-automation MCP is pure redundant tool-schema cost with no
  // functional gain (mirrors grimore-v2's D-011 addendum, 2026-08-21).
} else {
