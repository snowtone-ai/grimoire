...
test("drop catalog has 36 unique entries, 12 per rarity", () => {
  assert.equal(DROP_CATALOG.length, 36);
  assert.equal(new Set(DROP_CATALOG.map((drop) => drop.id)).size, 36);
  assert.equal(COMMON_DROPS.length, 12);
...
test("decideRarity: rate bands and first-of-day floor", () => {
  const context = { rollsSinceSsr: 0, isFirstOfDay: false };
  assert.equal(decideRarity(() => 0.0, context), 8);
  assert.equal(decideRarity(() => 0.049, context), 8);
  assert.equal(decideRarity(() => 0.05, context), 4);
  assert.equal(decideRarity(() => 0.319, context), 4);
  assert.equal(decideRarity(() => 0.32, context), 1);
