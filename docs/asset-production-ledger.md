...
Current phase: the feminine storybook expansion is complete: two Area heroes and 94 RARE 1–7 item masters were added, processed, and catalog-validated. Final application/browser verification is recorded in `docs/decisions.md`.
...
- Area heroes: the ten areas reachable from the Record page. `garden` is out of scope for Area hero generation.
...
| 3 | 90 | Terra / high–max |
| 4 | 48 | Terra / high–max |
| 5 | 70 | Terra / max; Sol review for important/flagged assets |
| 6 | 60 | Sol / xhigh; max for important assets |
| 7 | 40 | Sol / max |
| **RARE 1–7** | **518** | rarity × visual complexity × importance |
| Area heroes | 10 | Terra / max or Sol / xhigh for demanding scenes |
...
Ten accepted lossless masters live under `public/area-heroes/master/`. Each master represents one coherent roughly 2×2-viewport world canvas; the exact center 50% × 50% also passes as a complete portrait composition. Shipping derivatives are generated by `scripts/process-area-assets.mjs`:
...
| `petal` | 1024×1536 | `806947c05014` | 1 | pass; feminine storybook confectionery-garden expansion |
| `lullaby` | 1024×1536 | `5a1ce65b3bbb` | 1 | pass; feminine celestial bedtime-fantasy expansion |

The original eight-Area production used 13 observed calls including the two approved pilot calls. The 2026-09-05 expansion added one accepted call for each of `petal` and `lullaby`, for ten final exploration masters in total.
...
- All 511 non-pilot RARE 1–7 working masters now exist under `public/item-rewards/master/`; the seven separately approved pilots remain under `public/item-rewards/pilot/`, yielding all 518 catalog assets.
- `scripts/process-item-assets.mjs` completed and generated the shipping thumbnail and inspection AVIF/WebP derivatives plus `public/item-rewards/manifest.json` for all 518 assets.
- `scripts/build-item-production-index.mjs` completed and generated `data/item-asset-index.json` for all 518 assets.
- Historical accounting through 2026-08-23 was 432 production calls for 417 production masters, plus eight pilot calls for seven pilot masters. The 2026-09-05 expansion added 94 accepted, individually named masters from 94 generation calls. Area accounting is 15 calls for ten final exploration masters including the two pilot references.
...
## 2026-09-05 feminine storybook expansion

- Added `petal` (花菓子宮殿の庭) and `lullaby` (夢織り星谷) with the same RARE 1/2/3/5/6/7 distribution as every existing expedition region: 11/10/9/7/6/4, or 47 items per Area.
- Added 94 square lossless item masters and two portrait Area masters. Representative RARE 6–7 masters and both Area masters received source-resolution visual review for silhouette, crop, material, text/logo absence, and regional coherence.
- `scripts/process-area-assets.mjs` accepted all ten Area masters with ten unique hashes. `scripts/validate-item-assets.mjs` accepted 518/518 assets, 518 unique master hashes, and all 2,072 responsive derivative files.
- Final distribution is R1 110, R2 100, R3 90, R4 48, R5 70, R6 60, R7 40. RARE 8 photographs and history identifiers were not changed.
