...
## Current
- 2026-08-19: T043 (D-045訂正) — [grimore-v2ブランチ] T042/D-045を訂正。
  「v2は既存アプリへの追加」は誤りで、正しくは**v2はほぼ0からの作り直し**
  であり、流用するのは作り直しが面倒な外部インフラ・認証情報のみ(Google
  Cloud Console OAuthクライアント設定、Gemini APIキー、Vercel本番URL)。
  タスク管理・カレンダーのUI/ドメインロジック・IndexedDBスキーマは作り
  直しの対象に含まれる可能性が高い。既存ユーザー(家族)のIndexedDBデータ
  の引き継ぎ方針は新たな未決定事項としてO章へ追加(D-041と同じ訂正追記
  パターンをD-045に適用)。Gmailインポート廃止/Googleカレンダーインポート
  維持の決定は変更なし。
