...
## D-045: Grimoire v2のスコープ確定 — 既存アプリへの追加、Gmailインポート廃止

- 日付: 2026-08-19
- 対象: architecture / scope / v2-kickoff
- 決定:
  1. v2は既存のGrimoire（旧Task Plant）アプリへの**追加**であり、作り直し
     ではない（`grimore-v2/Grimoire_決定事項ログ.md` Q章に記載）。タスク
     管理・カレンダーの基本機能、IndexedDB（Dexie）の既存スキーマと
     ドメインロジック、`.env.local`の既存の値（GEMINI_API_KEY、Google
     OAuth関連等）をすべてそのまま流用する。新規に作るのは主に「相棒＋
     世界」画面レイヤーであり、既存2画面（ホーム/タスク管理、カレンダー）
     のドメインロジックを置き換える計画ではない。
  2. 例外としてGmailインポート機能（`src/components/gmail/gmail-import-
     modal.tsx`、`src/lib/api/gmail-task-extractor.ts`、`/api/gemini/
     generate`の`kind:"gmail"`経路）はv2で廃止する。Googleカレンダー
     インポート機能は現状維持。
- 採用理由: 相談セッション中にオーナーが明示。曖昧なまま暗黙の前提に
  なっていた「v2は既存アプリと別物を作るのか、追加なのか」を明文化する
  ことで、H章のモジュール分離方針・実装フェーズの統合点判断の前提を固定
  する。
- 不採用案: v2をゼロから作り直す（新規リポジトリ／新規データ層） →
  オーナーが明示的に既存流用を選択したため不採用。
- 将来見直し条件: なし。実際のGmailインポート機能のコード削除は実装
  フェーズの個別タスクとして扱う（本決定の時点では製品コード非変更）。
- 訂正（2026-08-19、同日中にオーナーが指摘）: 上記「決定1」（既存アプリへの
  追加であり作り直しではない／タスク管理・カレンダー・IndexedDBスキーマ・
  ドメインロジックをそのまま流用）は誤り。正しくは、**v2はほぼ0からの
  作り直し**であり、流用するのは作り直しが面倒な**外部インフラ・認証情報
  のみ**（Google Cloud Console のOAuthクライアント設定、Gemini API
  キー、Vercelの本番URL）。タスク管理・カレンダーのUI／ドメインロジック・
  IndexedDBスキーマは作り直しの対象に含まれる可能性が高い。既存ユーザー
  （家族）のIndexedDBデータをどう引き継ぐかは新たな未決定事項としてO章へ
  追加した。決定2（Gmailインポート廃止／Googleカレンダーインポート維持）
  はこの訂正後も変更なし。最新の正しい内容は
  `grimore-v2/Grimoire_決定事項ログ.md` Q章を参照。
