...
## D-016: Kenney "Interface Sounds"をパック全量(100件)へ拡張し、選定用マニフェストを同梱する

- 日付: 2026-08-22
- 対象: audio / assets / AI-assisted development
- 決定: D-015で12件だけ選んで導入したKenney "Interface Sounds"を、同じ
  CC0パックの全100件へ拡張して`public/audio/ui/`へ同梱する(新規88件、
  D-015当時の12件はファイル不変)。あわせて`public/audio/ui/manifest.json`
  を新設し、全100件それぞれのcategory(back/bong/click/close/confirmation/
  drop/error/glass/glitch/maximize/minimize/open/pluck/question/scratch/
  scroll/select/switch/tick/toggle)・用途hint・再生時間・現在`sound-catalog.ts`
  で使用中かどうかを機械可読で記録する。オーナー指示「フロントエンドの
  UIコンポーネントアセットのように、AIがどれを使うか完全に判断できる
  スタイル」に沿い、新しいキューを追加するたびにライセンス調査や外部
  ダウンロードをやり直す必要をなくす。フォーマットは既存12件と同じ
  PCM s16le/44.1kHz/monoのwavへ統一(D-015の理由と同じくSafari/iOSの
  `decodeAudioData`がogg/vorbisを確実にデコードできないため)。
  `public/sw.js`のruntime cacheコメントを実態(配線済みのみfetchされる、
  未配線分はビルドサイズにのみ計上)に合わせて修正
  (`src/audio/sound-catalog.ts`・実際の12キューは無変更)。
- 適用範囲: 本リポジトリ(grimore-v2系)のみ。`main`ブランチは別途、
  同じ100件+manifestを"配置のみ"で導入し、D-022の合成音エンジンは
  変更しない(オーナー指示で明示的に安全側を選択)。
- 採用理由: (a) D-015の「将来見直し条件」がまだ発火していない(パックが
  尽きたわけではない)段階での先行拡張だが、パック自体はD-015で既に
  ライセンス確認済みのCC0なので、追加のライセンス判断は不要、
  (b) 12件だけの部分導入だと新規キューを足すたびに「zipを再取得して
  該当ファイルだけwav変換する」という手作業が発生し、AIが自律的に
  選ぶ体験を阻害していた、(c) 追加88件でも約2.75MB、runtime cacheは
  配線済みファイルしかfetchしないため未配線分がPWAの実効オフライン
  容量を圧迫しない。
- 不採用案: (a) 12件のまま維持し、キュー追加のたびに都度取得
  → 手作業のたびにライセイン再確認と変換が発生し、まさに解消したい
  非効率そのもの。(b) パック全量をogg(元フォーマット)のまま同梱
  → サイズは小さくなるが、配線時に毎回wav変換の手作業が挟まり、
  かつ未変換のまま誤配線するとSafariで無音になる罠を残すため不採用。
- 将来見直し条件: D-015と同じく、図鑑の素材別効果音でこのパックの
  音色が不足した場合に同じCC0条件の追加パックを検討する。manifestの
  生成に使った変換手順はワンショットのため恒久スクリプト化はしない
  (再拡張が必要になった時点で同じ手順を再実行すれば足りる)。
