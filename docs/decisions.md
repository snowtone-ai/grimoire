...
## D-036: アイテムコレクション ビジュアル抜本刷新(プロトタイプ) — react-three-fiber製3D詳細ページ+エフェクト系
- 日付: 2026-08-16
- 対象: reward / ui / 3d / effects / assets
- 背景: ユーザーから、報酬アイテムの体験を全面刷新したいという依頼。現状は「獲得した瞬間」に
  価値が集中し、獲得後に1つ1つ眺めて楽しむ対象になっていない。要望は7点: (1)アイテム単体を
  超高品質なビジュアルにする、(2)記録ページでタップ→専用詳細ページ(モンハン装備箱のような
  観察・説明・パラメータ表示)、(3)アイテムが動く(常時アニメーション/タップで動く)、
  (4)アプリ起動時に複数種のランダムな美麗エフェクト、(5)12枚の花写真(RARE4正典+RARE8)
  以外は世界観ごと全面刷新可、モンハン的自然ファンタジー〜エルデンリング的ダークファンタジーを
  物語整合性を気にせず混在、(6)フレーバーテキストをモンハン装備解説文調に、(7)タスク完了
  エフェクトも魔法・剣戟調に複数種。
- 事前確認(3問): 予算は「無料素材+コード生成のみ」(Claude Pro月額のみという既存のグローバル
  制約と整合、有料API/AI画像生成は不可)、アニメーション技術は当初PixiJS(2D)を提案したが
  ユーザー指摘により再検討しThree.js系を採用、進め方は「まず設計+少数プロトタイプで方向性を
  確認」を選択。ユーザーから追加で「実在ゲームの技術調査をしてプロトタイプと比較しろ」
  「MCP/外部ツールを追加してよい」「UI/UXはAIが一から考えるより既存の優れた人間の
  プロダクトを模倣する方が確実に良い」との指示があり、設計を以下の通り確定した。
- 決定(技術スタック): **react-three-fiber + @react-three/drei**(MIT、React19/Next16対応)を
  新規導入。3Dモデルは**Kenney**の無料3Dアセットパック(Mini Dungeon 2.0/Graveyard Kit 5.0/
  Nature Kit、すべてCC0、表記義務なし)から選定。UIキットも当初Quaternius(itch.ioの
  pay-what-you-want購入フロー経由でしか配布されておらず自動取得不可と判明)やライセンス
  不明瞭な他社ダークファンタジーUIキットを検討したが、最終的にKenney "Fantasy UI Borders"
  (CC0、kenney.nlから直接ダウンロード可)に一本化——1作者・1ライセンス規約に統一することで
  出典管理の負担を下げた(docs/asset-sources.md)。Google製`<model-viewer>`も検討したが、
  タップ演出・完了エフェクトを同一WebGLシーン内でパーティクル/ライティングと統合したいため
  react-three-fiberを採用。
- 決定(データモデル): `DropDef`に`domain?`/`model?`/`effect?`/`params?`を追加(すべて任意
  フィールド、既存436件は無変更で旧描画のまま)。`region`は既存8地域+"garden"のまま変更せず、
  新規アイテムには`region: "fantasy"`という1つのセンチネル値を追加——`drop-catalog.test.mjs`
  の「region実在チェック」「RARE4/8はgardenのみ」という既存不変条件を無改変で満たしつつ、
  実際のグルーピングは新設`src/lib/domain/domains.ts`の`domain`で行う。
- 決定(エフェクト設計): 436件規模でも無料で成立させるため、アイテムごとの個別アニメーションは
  作らず、5種の共有パーティクル「レシピ」(`radiant`/`arcane`/`ember`/`verdant`/`void`、
  `src/lib/three/domain-recipes.ts`)をdomain×レアリティ帯(sound.tsと同じ1-3/4-6/7-8の
  tier分け)で選択し、アイテム固有色でティントする方式に統一。レシピの実体はdreiの
  `<Sparkles>`(既存の実証済みパーティクル実装を流用、自前でパーティクル物理を書かない)。
- 決定(3画面): (1)アイテム詳細ページは`src/app/book/[dropId]/page.tsx`という新規動的
  ルート(モーダルではなく実ページ) — 理由は既存の`next-view-transitions`のページめくり
  演出と`playPage()`効果音(従来ほぼ未活用)をそのまま活かせる、戻るボタンでアイテム間を
  辿れる、未発見アイテムは名前もモデルも見せず「？？？」を維持できる。(2)アプリ起動エフェクトは
  `sessionStorage`で1セッション1回、`prefersReducedMotion()`で完全スキップ、タップでスキップ
  可。(3)タスク完了エフェクトは`drop-reveal.tsx`に直接統合(既存の`fireDropConfetti`と並走、
  置き換えは今回見送り)——当初案ではuse-home-screen.tsに新規呼び出しを追加する設計だったが、
  DropRevealが`grant.drop`を既に持っているためレンダリング側で完結させる方がシンプルと判断。
- 決定(コンテンツ): 3ドメイン(灰燼の刃庭=embercinder/虚ろの瘴湿地=hollowmire/棘紗の森=
  thornveil)×8アイテムのプロトタイプを新規`src/lib/domain/reward-catalog/fantasy.ts`に
  追加。モンハン装備解説文調のフレーバーテキストを新規執筆。既存8地域カタログファイル・
  428件の旧アイテムは無改変。
- 実装中に発見した不具合と修正: mini-dungeon/graveyard-kit由来のGLBは相対パス
  `Textures/colormap.png`で自パックのテクスチャアトラスを外部参照しており(GLB化されても
  埋め込まれない)、2パックの同名ファイルが異なる中身のため単純に1ディレクトリへ混在させると
  衝突する。`public/models/items/{mini-dungeon,graveyard-kit,nature-kit}/`とパックごとに
  分離して解決(docs/asset-sources.md)。
- 検証: `pnpm verify`全緑(lint/typecheck/74テスト/build)。Playwrightでモバイルビューポート
  (390x844)を用いた実機相当の確認——起動フラッシュ、/book一覧のクリック遷移、アイテム
  詳細ページの3Dモデル回転+タップ再演出、タスク完了エフェクトをすべて目視確認。確認作業中に
  T029と同種の「開発中のService Workerがコード変更前のJSを配信し続ける」誤検知に遭遇し、
  毎回SW登録解除+キャッシュクリアで対処(製品バグではなくローカル検証手順上の既知の罠として
  記録)。
- IP境界: モンスターハンター/エルデンリングいずれも固有名詞・アセット・UIレイアウトの直接
  模倣はせず、借用したのは「回転する3Dモデル+動的ライティング+装飾枠+パラメータ表記+
  読み物としてのフレーバーテキスト」という一般的なジャンル表現と、実在の3D/UI素材(CC0で
  合法的に再配布可能なもの)のみ。著作権のある実機スクリーンショットは取得・複製していない
  (WebSearchでの技術調査に留め、比較検証はPlaywrightで撮影した本アプリ自身のスクリーンショット
  をユーザーに提示する形で代替)。
- 不採用案: PixiJS(2D)ベースの当初案(ユーザー指摘により3D方向へ転換)、game-icons.net
  (CC BY 3.0、個別アイコンごとの著作者表記管理が436件規模で煩雑になるため、表記不要な
  CC0のKenneyへ一本化)、Quaternius(自動ダウンロード経路がなく実装時に取得不可と判明)、
  436件全体の即時書き換え(方向性未確定のまま大規模書き換えするリスクを回避)。
- 将来見直し条件: ユーザーがプロトタイプの方向性を承認した場合、残り428件・既存8地域への
  展開方針(regionとdomainの統合可否含む)を次のタスクで決定する。Tier 1/2セルフレビューは
  方向性確定後に実施(スコープがまだ変わりうるため今回は見送り)。
