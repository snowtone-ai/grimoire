/**
 * Ported from v1 (main branch) src/lib/api/google-auth.ts — see D-013.
...
/**
 * v2 addition (T020): whether Google linking is configured at all. Callers
 * use this to treat an unset NEXT_PUBLIC_GOOGLE_CLIENT_ID as "integration
 * disabled" and stay quiet, instead of surfacing the throw below as an
 * error. requestGoogleToken still throws when called without a client id —
 * a caller that skips this check is not silently swallowed.
 */
