...
| `src/quality.js` | p20 FPS と描画予算による純粋な品質 governor、hysteresis、session lock |
| `src/scene.js` | 組み立て、品質計測、H章の環境プリセット契約 v2 の発行 |
...
| G/S章「性能判定で full / reduced を決定」 | 120 frame の p20 FPS と scene draw / triangle / post-pass 予算を監視。warmup・hysteresis・cooldownを経て昇降格し、2回目の縮退後はsession中reducedへlock |
...
| 三角形 | 150,000（受入許容 +2%、実効上限153,000） | 152,490 | 44,918 |

`renderer.info.render.calls` が示す 67 は、シーン 29 に光芒の遮蔽バッファ用の再描画 29 と
ポストプロセス 9 を足したもの。シーン自体の draw call は予算内。

三角形は基準予算を 1.7% 超過するが、T008 の受入許容 +2% 内である。品質 governor は
実効上限153,000を超えたときだけ triangle pressure とし、この実測値だけでは縮退しない。
...
- 品質 governor の draw call は sky + world の primary scene calls を使う。遮蔽再描画を含む
  multipass total は診断値、fullscreen work は `postPasses` として別に扱い、混同しない。
