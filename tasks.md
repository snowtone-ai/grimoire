...
| T046 | done | Claude Code | - | scripts/setup.mjs(1関数のみ), docs/decisions.md, docs/state.md, tasks.md | main側のpm-zero v12.1追随作業(T043/D-046)で発見したバグを本ブランチにも適用する。hasShadcn()のidempotency判定が.claude/skills/shadcn-uiを見ていたが、npx skills add shadcn/uiが実際に書き込むパスは.claude/skills/shadcn(.agents/skills/shadcnへのsymlink)であり、常にfalseを返し再実行のたびに再インストールを試みる。本ブランチのpackage.jsonはshadcnに未依存のため未発火(dormant)だったが、main側で先に修正・確認済みのため揃えた | node scripts/setup.mjsが例外なく完走すること、git diff --check | 2026-08-23: .claude/skills/shadcn-ui -> .claude/skills/shadcnに修正し、判定falseの分岐にalready presentログを復元(mainと同一の分岐構造)。node scripts/setup.mjsを本ブランチのworktreeで実行し正常終了を確認(shadcn分岐はpackage.json未依存のため未発火、impeccable分岐のみ発火)。 |
